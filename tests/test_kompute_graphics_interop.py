...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Kompute graphics interop example and compare produced frames against golden images with FLIP."
    )
    parser.add_argument("--example", required=True, help="Path to the interop example executable")
...
def main() -> int:
    ...
    if not example.is_file():
        raise RuntimeError(f"interop executable does not exist: {example}")
    ...
    if failures:
        raise AssertionError("interop frame comparison failed:\n" + "\n".join(failures))
